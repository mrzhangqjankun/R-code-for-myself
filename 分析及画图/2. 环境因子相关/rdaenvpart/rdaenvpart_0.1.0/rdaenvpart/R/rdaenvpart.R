...
#'
#' This function calculate the individual explain percentage of each environmental variable for Canonical Correspondence Analysis and Redundancy Analysis,
    ...
    if (Env.num > 13)
	   ...
	   if (pieplot=="tv") {
		...
		if (pieplot=="tev") {
           ...
           lbls <- paste(lbls, pct) # add percents to labels
           lbls <- paste(lbls,"%",sep="") # ad % to labels
           pie(pct,labels = lbls, main="individual % on total explained variation")
        }
