...
#spp: 转置且重抽过的OTU。A community table for communities of interest with local communities/samples as rows and taxa as columns. All samples must be rarefied to the same depth.
#pool: 源群落。A community table for defining source community (optional; Default=NULL).
#taxon: 物种信息。A table listing the taxonomic calls for each otu, with OTU ids as row names and taxonomic classifications as columns.
...
###自己加的内容------------------
#注意，OTU一定要是重抽过的，否则就会报下面的错！！
setwd("E:/桌面/test.data/")
spp = read.table(file="Galaxy296-[resample_normalized_uparse_otu_ALS.tabular].txt",sep="\t",row.names = 1,header = T)
spp = t(spp)
###------------------
...
sncm.fit <- function(spp, pool=NULL, stats=TRUE, taxon=NULL){
  ...
  bino.LL <- function(mu, sigma){
    R = freq - pbinom(d, N, p, lower.tail=FALSE)  #这里都是NaN，下面会报错。应该是数据问题
    R = dnorm(R, mu, sigma)
    -sum(log(R))
  }
  bino.mle <- mle(bino.LL, start=list(mu=0, sigma=0.1), nobs=length(p))
  ####--------------------
  #这会报错Error in optim(start, f, method = method, hessian = TRUE, ...) : 
  # 'vmmin'的初始值不能为无穷大
  #改成下面的就可以了
  #?pbinom #二项分布
  #?dnorm  #正态分布
