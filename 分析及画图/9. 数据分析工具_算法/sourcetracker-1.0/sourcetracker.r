...
rm(list=ls(all=TRUE))
setwd('D:/文件存放/galaxy pipeline/galaxy/自己整理代码/R code/sourcetracker-1.0/')
...
common.sample.ids <- intersect(rownames(metadata), rownames(otus)) ##取交集
...
results <- predict(st,otus[test.ix,], alpha1=alpha1, alpha2=alpha2) ###特别的慢
